...
-- 2. 비교 연산자
-- 좌항과 우항을 비교 
-- WHERE절에서 자주 사용됨, 원하는 레코드를 정확히 조회하는데 중요한 역할을 수행.
...
-- <=> : 좌항과 우황이 모두 null 이면 true
SELECT * FROM transaction
WHERE note <=> complete;

-- IS : 좌항이 우황과 같으면 true (키워드) 
...
-- BEETWEEN min AND max : min보다 크거나 같으면서 max 보다 작거나 같으면 true
-- NOT BEETWEEN min AND max : 좌항이 min보다 작거나 max보다 크면 true
SELECT * FROM transaction 
...
-- ORDER BY : 조회 결과를 특정 컬럼 기준으로 정렬 / 정렬을 선택하지 않으면 기본으로 ASC로 선택된다.
-- 1. ASC(오름차순 정렬) 
-- 2. DSC(내림차순 정렬)
SELECT * FROM transaction
ORDER BY amount ASC;
