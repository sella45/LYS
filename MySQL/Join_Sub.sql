...
-- 서브쿼리 : 쿼리 내부에서 존재하는 또 다른 쿼리, 쿼리 결과를 조건이나 테이블로 사용할 수 있도록 함.
...
-- WHERE 조건절에서 서브쿼리를 사용할 떈 연산자에 따라 에코드의 개수를 잘 확인해야함.
SELECT employee_number, name, age
FROM employee
...
-- 서브쿼리를 FROM 절에서 사용할 땐 3개 이상의 테이블을 조인해서 결과를 얻고자 할 때 유용하게 사용됨
