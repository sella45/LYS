...
// - useEffect : 컴포넌트의 생명주기에 따라 특정 작업을 수행할 수 있도록 하는 함수 (mount, update, unmoint에 해당하는 생명주기에 따라 실행)
// - useRef : 특정 데이터를 기억하기 위한 함수 값이 변경되더라도 렌더링을 수행하고싶지 않을 때 DOM요소를 기억하고 싶을 때 사용  
// - useContext : 컴포넌트 트리 상에서 전역 상태를 공유하고자 할 때 사용하는 함수 
// - useReducer : 상태와 관련된 로직을 관리하는 함수(싱태 업데이트 함수를 컴포넌트 외부로 옮길 수 있도록하여 코드관리를 수월하게 도와줌)
// - useCallbaack : 메모리 상에서 함수를 재호출할 수 있도록 하는 함수 (렌더링 혹은 이벤트 없이 특정 함수를 호출할 때 )
// - useMemo : 메모리 내에 함수의 결과를 저장하여 함수 재호출하지 않고 결과를 사용할 수 있도록 하는 방식 

// - hook함수는 반드시 함수형 컴포넌트 코드 블럭 라인에 존재해야함 
...
export default function HookComponent1() {
    ...
    // - hook함수는 반드시 컴포넌트 코드블럭 라인에 존재해야함
    // const func = () => {
    //     const [state, setState] = useState<number>(0);
    // }
    ...
    // - update : 컴포넌트 상태가 변경 혹은 지정되면서 렌더링되었을 때 
    ...
    // 스코프할 상태 배열을 지정하지 않으면 렌더링될 떄마다 실행됨
    // useEffect(()=>{
    ...
    useEffect(() => {
        ...
        // 스코프할 상태 배열에 지정된 상태를 해당 effect 내에서 변경하면 무한 호출 됨 
        // setState(state + 1);
    }, [state]);
